...
def plot_dupont_decomposition(dupont_history: Dict[int, Dict]) -> go.Figure:
    ...
    """
    years = sorted(dupont_history.keys())
    nm = [dupont_history[y].get("净利率") for y in years]
    ato = [dupont_history[y].get("总资产周转率") for y in years]
    em = [dupont_history[y].get("权益乘数") for y in years]
    roe = [dupont_history[y].get("ROE (杜邦计算)") for y in years]
